ice.aknnreg <- function(y, x, a, k, apostasi = "euclidean",
                        rann = FALSE, ind = 1, frac = 0.2, qpos = 0.9) {
  ...
  for (i in 1:nu) {
    X <- x
    X[, ind] <- xsel[i]
    est[i, ] <- Rfast::colmeans( Compositional::aknn.reg(X, y, x, a, k, apostasi, rann)[[ 1 ]][[ 1 ]] )
  }
